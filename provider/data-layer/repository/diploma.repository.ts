...
export class DiplomaRepository{
    ...
    public async get(providerId: string | undefined): Promise<Diploma[] | null>{
        DiplomaRepository._connection = await DatabaseUtils.getConnection();
        try {  
            if(DiplomaRepository._connection && providerId !== undefined){
                const res = await DiplomaRepository._connection.query(`SELECT * FROM ${this.table} WHERE providerId = "${providerId}"`);
                const data = res[0];
    ...
    public async insert(diploma: Diploma): Promise<Diploma | null> {
        DiplomaRepository._connection = await DatabaseUtils.getConnection();
        try {
            if(DiplomaRepository._connection){
    ...
    public async update(id: string | undefined, diploma: Diploma): Promise<Diploma | null> {
        DiplomaRepository._connection = await DatabaseUtils.getConnection();
        try {
            if(DiplomaRepository._connection && id !== undefined){
